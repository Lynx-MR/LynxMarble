Shader "MarbleGame/Fresnel_Masked" {
    //show values to edit in inspector
    Properties {
        _Color ("Tint", Color) = (0, 0, 0, 1)
        _SecondColor ("Tint 2", Color) = (0, 0, 0, 1)
        ...
        _SecondFresnelColor ("Fresnel Color 2", Color) = (1,1,1,1)
    ...
    SubShader {
        ...
        void surf (Input i, inout SurfaceOutputStandard o) {
            ...
            float2 uvOffset = float2(_Time.y/2.0, -_Time.y);
            fixed4 mask = tex2D(_MainTex, i.uv_MainTex + uvOffset);
            float4 col = _Color;
            float4 secondCol = _SecondColor;
            o.Albedo = lerp(col.rgb, secondCol.rgb, mask.x);
            ...
            float3 secondfresnelColor = fresnel * _SecondFresnelColor;
            //apply the fresnel value to the emission
            o.Emission = _Emission + lerp(fresnelColor, secondfresnelColor, mask.x);
        }
        ENDCG
    }
    FallBack "Standard"
